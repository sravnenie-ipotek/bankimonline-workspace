...
Our refinance credit testing infrastructure automatically creates detailed bilingual bug reports in Jira whenever test failures occur. Each bug includes exact file paths, comprehensive action logs, structured test steps, complete environment context, and automatic screenshot attachment.

Наша инфраструктура тестирования рефинансирования кредита автоматически создает подробные двуязычные отчеты о багах в Jira при возникновении сбоев тестов. Каждый баг включает точные пути к файлам, подробные журналы действий, структурированные шаги тестов, полный контекст окружения и автоматическое прикрепление скриншотов.
...
Issue Type: Баг
Summary: [Cypress] Refinance credit - break-even analysis calculation error
Labels: [cypress, auto-filed, refinance-credit, automated-qa, bilingual]
...
├──────────────────────────────────────────────────────────────────────────┤
│ Spec File | Файл спецификации  │ cypress/e2e/refinance-credit/step3.cy.ts │
│ Test Title | Название теста     │ Refinance Credit Step 3 - Break-even    │
│ File Path | Путь к файлу       │ cypress/e2e/refinance-credit/step3.cy.ts │
│ Current URL | Текущий URL       │ /services/refinance-credit/3             │
└──────────────────────────────────────────────────────────────────────────┘

🖥️ Environment | Окружение
• Browser | Браузер: Chrome 120.0.6099.129
• Operating System | Операционная система: darwin x64
• Timestamp | Время: 2024-01-15T15:45:18.456Z

❌ Error Details | Детали ошибки
AssertionError: expected break-even months '18' to equal '15'
    at Context.<anonymous> (cypress/e2e/refinance-credit/step3.cy.ts:67:89)

📋 Test Steps | Шаги теста
1. ✅ visit: /services/refinance-credit/3
2. ✅ get: [data-testid="current-loan-balance"]
3. ✅ type: 250000
4. ✅ get: [data-testid="current-interest-rate"]
5. ✅ type: 5.5
6. ✅ get: [data-testid="new-interest-rate"]
7. ✅ type: 4.2
8. ❌ should: break-even calculation, expected 15 but got 18

🔍 Action Log | Журнал действий
[2024-01-15T15:45:15.000Z] Test started: Refinance Credit Step 3 - Break-even
[2024-01-15T15:45:15.200Z] visit: ["/services/refinance-credit/3"]
[2024-01-15T15:45:15.800Z] get: ["[data-testid=\"current-loan-balance\"]"]
[2024-01-15T15:45:16.100Z] type: ["250000"]
[2024-01-15T15:45:16.400Z] get: ["[data-testid=\"current-interest-rate\"]"]
[2024-01-15T15:45:16.700Z] type: ["5.5"]
[2024-01-15T15:45:17.000Z] get: ["[data-testid=\"new-interest-rate\"]"]
[2024-01-15T15:45:17.300Z] type: ["4.2"]
[2024-01-15T15:45:17.600Z] should: ["contain.text","15 months"]
[2024-01-15T15:45:17.800Z] FAILED - should: AssertionError: expected 15 but got 18
[2024-01-15T15:45:18.000Z] URL changed to: /services/refinance-credit/3
[2024-01-15T15:45:18.400Z] Test failed: Break-even calculation mismatch

🔧 Debug Information | Отладочная информация
• Fingerprint | Отпечаток: cfp_b2c3d4e5f6
...
JIRA_LABELS=cypress,auto-filed,refinance-credit  # Additional labels | Дополнительные метки
```

**Test Execution Commands | Команды выполнения тестов:**
```bash
# Run all refinance credit tests with Jira integration
# Запустить все тесты рефинансирования кредита с интеграцией Jira
cd mainapp
npm run cypress:run -- --spec "cypress/e2e/**/refinance-credit*.cy.ts"

# Run specific refinance step with enhanced logging
# Запустить конкретный шаг рефинансирования с расширенным логированием
npm run cypress:run -- --spec "cypress/e2e/refinance-credit/step*.cy.ts" --env jiraEnabled=true
...
🎯 Jira bug created/updated: https://bankimonline.atlassian.net/browse/TVKC-43
📍 Bug details: https://bankimonline.atlassian.net/browse/TVKC-43

# Verification commands | Команды проверки
curl -u email:token https://bankimonline.atlassian.net/rest/api/3/issue/TVKC-43
```
...
2. **Monitor Auto-Filed Bugs | Мониторинг автоматически поданных багов:**
   ...
   - Filter by labels: `cypress`, `auto-filed`, `refinance-credit`
...
1. **Bug Analysis | Анализ багов:**
   - Review exact file path: `cypress/e2e/refinance-credit/step*.cy.ts`
...
2. **Reproduction Steps | Шаги воспроизведения:**
   ...
   code mainapp/cypress/e2e/refinance-credit/step3.cy.ts:67
   
   # Run single test for debugging | Запустить один тест для отладки
   cd mainapp
   npm run cypress:open -- --spec cypress/e2e/refinance-credit/step3.cy.ts
   ```

3. **Fix Verification | Проверка исправления:**
   ```bash
   # Re-run specific test | Повторно запустить конкретный тест
   npm run cypress:run -- --spec "cypress/e2e/refinance-credit/step3.cy.ts"
...
**Refinance Credit Specific Metrics | Метрики рефинансирования кредита:**

```yaml
Test Coverage | Покрытие тестов:
  - Current loan balance validation: 96% | Проверка баланса текущего кредита: 96%
  - Interest rate comparisons: 94% | Сравнения процентных ставок: 94%
  - Break-even calculations: 91% | Расчеты точки безубыточности: 91%
  - Monthly payment projections: 93% | Прогнозы ежемесячных платежей: 93%
  - Savings calculations: 89% | Расчеты экономии: 89%

Bug Categories | Категории багов:
  - Calculation Logic: 50% | Логика расчетов: 50%
  - Data Validation: 25% | Проверка данных: 25%
...
- **Refinance Credit Bugs**: https://bankimonline.atlassian.net/issues/?jql=project%20%3D%20TVKC%20AND%20labels%20%3D%20refinance-credit
...
This comprehensive integration ensures that every refinance credit test failure is automatically documented with precise technical details, complete debugging context, and bilingual descriptions for optimal developer and QA team collaboration.

Эта комплексная интеграция гарантирует, что каждый сбой теста рефинансирования кредита автоматически документируется с точными техническими деталями, полным контекстом отладки и двуязычными описаниями для оптимального сотрудничества команд разработчиков и QA.