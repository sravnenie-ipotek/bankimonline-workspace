...
const persistedRefinanceMortgageReducer = persistReducer(refinanceMortgagePersistConfig, refinanceMortgageReducer);
const persistCreditConfig = {
    key: 'calculateCredit',
    storage,
};
const persistedCreditReducer = persistReducer(persistCreditConfig, creditReducer);
const refinanceCreditPersistConfig = {
    key: 'refinanceCredit',
    storage,
};
const persistedRefinanceCreditReducer = persistReducer(refinanceCreditPersistConfig, refinanceCredit);
const borrowersPersonalDataPersistConfig = {
    key: 'borrowersPersonalData',
    storage,
};
const persistedBorrowersPersonalDataReducer = persistReducer(borrowersPersonalDataPersistConfig, borrowersPersonalDataReducer);
const otherBorrowersConfig = {
    key: 'otherBorrowers',
    storage,
};
const persistedOtherBorrowersReducer = persistReducer(otherBorrowersConfig, otherBorrowersReducer);
const borrowersPersistConfig = {
    key: 'borrowers',
    storage,
};
const persistedBorrowersReducer = persistReducer(borrowersPersistConfig, borrowersReducer);
