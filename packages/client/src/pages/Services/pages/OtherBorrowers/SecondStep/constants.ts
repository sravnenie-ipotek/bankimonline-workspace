...
export const getValidationSchema = () => Yup.object().shape({
  ...
  startDate: Yup.string().when('mainSourceOfIncome', {
    is: (value: string) => !['unemployed', 'unpaid_leave', 'other'].includes(value), // Not unemployed, no income, or other
  ...
  fieldOfActivity: Yup.string().when('mainSourceOfIncome', {
    is: (value: string) =>
      value !== null && value !== undefined && value !== '' && !['unemployed', 'unpaid_leave', 'other'].includes(value),
    then: (shema) =>
