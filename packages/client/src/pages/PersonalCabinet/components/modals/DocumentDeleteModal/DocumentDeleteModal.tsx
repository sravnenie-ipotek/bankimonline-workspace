  ...
  return (
    <Modal isVisible={isOpen} onCancel={handleCancel} className={cx('modal-container')}>
      <div className={cx('modal-content')}>
        {/* Action 3: Modal Header with Title */}
        <div className={cx('modal-header')}>
          ...
          <h2 className={cx('modal-title')}>
            {t('delete_document_confirmation', 'Вы уверены, что хотите удалить документ?')}
          </h2>
          
          {documentName && (
            <p className={cx('document-name')}>
              {documentName}
            </p>
          )}
        </div>
