 ...
 */
export const getAdminProfile = createAsyncThunk('admin/getProfile', async (_, { getState, rejectWithValue }) => {
    try {
 ...
 */
export const getAdminStats = createAsyncThunk('admin/getStats', async (_, { getState, rejectWithValue }) => {
    try {
        const state = getState();
        const token = state.admin.token;
        if (!token) {
...
export const selectAdminIsAuthenticated = (state) => state.admin.isAuthenticated;
