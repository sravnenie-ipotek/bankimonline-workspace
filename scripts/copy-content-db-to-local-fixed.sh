...
# Content Database Copy Script (Fixed Version)
# Copies content database from Railway to local PostgreSQL

set -e  # Exit on any error

echo "🚀 Content Database Copy Script (Fixed Version)"
echo "==============================================="
...
# Backup Railway database (simplified version)
backup_railway_database() {
    ...
    # Create backup with schema and data (simplified)
...
# Restore to local database (simplified version)
restore_to_local() {
    ...
    if PGPASSWORD=$LOCAL_PASSWORD psql -h $LOCAL_HOST -p $LOCAL_PORT -U $LOCAL_USER -d $LOCAL_DB -f "$BACKUP_FILE" 2>/dev/null; then
        ...
        print_status "Trying alternative restore method..."
        
        # Try alternative method with different flags
        if PGPASSWORD=$LOCAL_PASSWORD psql -h $LOCAL_HOST -p $LOCAL_PORT -U $LOCAL_USER -d $LOCAL_DB -v ON_ERROR_STOP=0 -f "$BACKUP_FILE" 2>/dev/null; then
            print_success "Database restored successfully (with warnings)"
        else
            print_error "All restore methods failed"
            exit 1
        fi
    fi
}
