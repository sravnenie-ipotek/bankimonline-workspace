...
async function fixStep3Simple() {
    ...
    try {
        ...
        // Create missing containers with minimal fields
        console.log('1. Creating missing dropdown containers...');
        
        // Main source container
        const mainSourceContainer = await client.query(`
            INSERT INTO content_items (screen_location, content_key, component_type, category, is_active)
            VALUES ('mortgage_step3', 'mortgage_step3.field.main_source', 'dropdown_container', 'mortgage', true)
            RETURNING id;
        `);
        ...
        const additionalIncomeContainer = await client.query(`
            INSERT INTO content_items (screen_location, content_key, component_type, category, is_active)
            VALUES ('mortgage_step3', 'mortgage_step3.field.additional_income', 'dropdown_container', 'mortgage', true)
            RETURNING id;
        `);
        ...
        const obligationsContainer = await client.query(`
            INSERT INTO content_items (screen_location, content_key, component_type, category, is_active)
            VALUES ('mortgage_step3', 'mortgage_step3.field.obligations', 'dropdown_container', 'mortgage', true)
            RETURNING id;
        `);
        ...
        // Update migration_status for new containers
        await client.query(`
            UPDATE content_items 
            SET migration_status = 'migrated'
            WHERE id IN ($1, $2, $3)
        `, [mainSourceId, additionalIncomeId, obligationsId]);
