...
export const otherBorrowersSlice = createSlice({
    ...
    reducers: {
        updateOtherBorrowers: (state, action) => {
            ...
            const additionalIncomeModal = state.otherBorrowers[index]?.additionalIncomeModal;
            const obligationModal = state.otherBorrowers[index]?.obligationModal;
            const sourceOfIncomeModal = state.otherBorrowers[index]?.sourceOfIncomeModal;
            if (index !== -1) {
        ...
        deleteOtherBorrowers: (state, action) => {
            state.otherBorrowers = state.otherBorrowers.filter((item) => item.id !== action.payload);
        },
        updateSourceOfIncomeModal: (state, action) => {
            ...
            const index = state.otherBorrowers[pageId - 1].sourceOfIncomeModal.findIndex((item) => item.id === id);
            if (index !== -1) {
        ...
        deleteSourceOfIncomeModal: (state, action) => {
            ...
            state.otherBorrowers[action.payload.pageId - 1].sourceOfIncomeModal =
                state.otherBorrowers[action.payload.pageId - 1].sourceOfIncomeModal.filter((item) => item.id !== action.payload.id);
        },
        updateAdditionalIncomeModal: (state, action) => {
            const { pageId, id } = action.payload;
            const index = state.otherBorrowers[pageId - 1].additionalIncomeModal.findIndex((item) => item.id === id);
            if (index !== -1) {
        ...
        deleteAdditionalIncomeModal: (state, action) => {
            const { pageId, id } = action.payload;
            state.otherBorrowers[pageId - 1].additionalIncomeModal =
                state.otherBorrowers[pageId - 1].additionalIncomeModal.filter((item) => item.id !== id);
        },
        updateObligationModal: (state, action) => {
            const { pageId, id } = action.payload;
            const index = state.otherBorrowers[pageId - 1].obligationModal.findIndex((item) => item.id === id);
            if (index !== -1) {
        ...
        deleteObligationModal: (state, action) => {
            const { pageId, id } = action.payload;
            state.otherBorrowers[pageId - 1].obligationModal = state.otherBorrowers[pageId - 1].obligationModal.filter((item) => item.id !== id);
        },
...
export const { updateSourceOfIncomeModal, deleteSourceOfIncomeModal, updateAdditionalIncomeModal, deleteAdditionalIncomeModal, updateObligationModal, deleteObligationModal, updateOtherBorrowers, deleteOtherBorrowers, openBorrowersPage, } = otherBorrowersSlice.actions;
