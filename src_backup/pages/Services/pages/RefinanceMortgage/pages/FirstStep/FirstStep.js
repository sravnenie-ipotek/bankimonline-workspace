...
export const validationSchema = Yup.object().shape({
    whyRefinancingMortgage: Yup.string().required(i18next.t('error_select_answer')),
    mortgageBalance: Yup.number()
        .test('is-less', 'Остаток по ипотеке не может быть больше чем стоимость недвижимости', function (mortgageBalance) {
    ...
    priceOfEstate: Yup.number()
        .test('is-greater', 'Полная стоимость недвижимости не может быть меньше чем остаток ипотеки', function (priceOfEstate) {
    ...
    mortgageData: Yup.array().test('is-balance-sum-equal', 'Сумма балансов ипотек не соответствует заданному значению', function () {
        const totalBalance = this.parent.mortgageData.reduce((sum, item) => sum + (item.balance || 0), 0);
...
const FirstStep = () => {
    ...
    return (_jsxs(_Fragment, { children: [_jsx(Formik, { initialValues: initialValues, validationSchema: validationSchema, validateOnMount: true, onSubmit: (values) => {
                ...
                }, children: _jsxs(Form, { children: [_jsxs(Container, { children: [_jsx(VideoPoster, { title: t('sidebar_sub_refinance_mortgage'), text: t('refinance_mortgage_promo_text'), size: "small" }), _jsx(FirstStepForm, {})] }), _jsx(SingleButton, {})] }) }), _jsx(LoginModal, {})] }));
};
